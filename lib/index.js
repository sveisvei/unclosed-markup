...
function getCharsUntilLine (contentUntilStart, startOffset) {
    let lines = contentUntilStart.split(/\n|\n\r/);
    let result = lines.reduce((currentLine, lineStr, index) => {
...
function findLineInfo (fileContent, startOffset, endOffset) {
    let contentUntilStart = fileContent.substring(0, startOffset);
    let line = contentUntilStart.match(/\n|\n\r/g);
    line = (line && line.length) || 0;


    let lines = fileContent.split(/\n|\n\r/);
    let lineStart = getCharsUntilLine(contentUntilStart, startOffset);
